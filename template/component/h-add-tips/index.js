...
Component({
  ...
  // 配置引入外部样式
  externalClasses: ['h-class', 'h-arrow-class','h-tips-icon'],
  ...
  ready: function () {    
    this.initTips(this.data.cacheTime);
  },
  /**
   * 组件的方法列表
   */
  methods: {
    initTips: function (cacheTime) {
      // 判断是否已经显示过
      let cacheOne = wx.getStorageSync(STORAGE_KEY_ONE);
      const now = +new Date();
      // 校验缓存数据 以及缓存时间是否过期(关闭后缓存一个月 一个月后重新提示用户)
      if (cacheOne && (now - cacheOne < cacheTime)) return;
      // 处理根据系统信息处理位移箭头位置（重点）
